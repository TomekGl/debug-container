...
```
docker run --rm -ti ghcr.io/tomekgl/debug-container:1.7.0
...
```
> docker run --rm -ti ghcr.io/tomekgl/debug-container:1.7.0 ping -c1 google.com
...
With Bash shell:
```
docker run --rm -ti ghcr.io/tomekgl/debug-container:1.7.0 /bin/bash
5a9eed15816a:/#
```

Running a Bash one-liner:
```
docker run --rm -ti ghcr.io/tomekgl/debug-container:1.7.0 /bin/bash -c 'echo $HOSTNAME && echo ${HOSTNAME^^}'
0776f193a7b9
0776F193A7B9
```
...
## As a regular pod
```
kubectl run my-debug-pod --rm -ti --image ghcr.io/tomekgl/debug-container:1.7.0
```

## Accessing Kubernetes node's root filesystem/network/PID namespaces
```
kubectl debug node/ip-172-16-1-2.eu-west-1.compute.internal -it --image=ghcr.io/tomekgl/debug-container:1.7.0
```
...
```
⋊> root@vpsc ⋊> ~ kubectl debug -it pod-under-test --image=ghcr.io/tomekgl/debug-container:1.7.0 --target=pod-under-test
